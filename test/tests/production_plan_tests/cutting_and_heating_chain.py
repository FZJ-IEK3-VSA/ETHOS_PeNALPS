...
def fill_cutting_and_heating_chain(
    ...
    furnace_heating_state = (
        open_hearth_furnace.process_state_handler.create_state_for_parallel_input_and_output_stream_with_storage(
            process_state_name="Heating"
        )
    )
    idle_furnace = open_hearth_furnace.process_state_handler.create_idle_process_state(process_state_name="Idle")
    ...
    cut_steel_stream.create_stream_energy_data(specific_energy_demand=100, load_type=electricity_load)
