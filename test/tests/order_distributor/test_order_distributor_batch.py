...
def test_batch_order_distribution():
    ...
    order_distributor.add_stream_name(stream_name=stream_1.name, process_chain_identifier=process_chain_identifier_1)
    order_distributor.add_stream_name(stream_name=stream_2.name, process_chain_identifier=process_chain_identifier_2)
    order_distributor.update_order_collection(new_order_collection=order_collection)
    list_of_meta_data = []
    total_order_collection_meta_data = post_process_order_collection(order_collection=order_collection)
    ...
    for splitted_order in order_distributor.dict_of_splitted_order.values():
        ...
        splitted_total_mass = splitted_order.order_data_frame.loc[:, "production_target"].sum()
