...
def fill_cutting_and_packaging_chain(
    ...
    # Cutting Machine
    cutting_state = (
        cutting_machine.process_state_handler.create_state_for_parallel_input_and_output_stream_with_storage(
            process_state_name="Continuous Cutting"
        )
    )

    idle_state = cutting_machine.process_state_handler.create_idle_process_state(process_state_name="Idle State")
    ...
    # Packaging machine
    packing_state = (
        packaging_machine.process_state_handler.create_state_for_parallel_input_and_output_stream_with_storage(
            process_state_name="Packaging"
        )
    )

    idle_state_packing = packaging_machine.process_state_handler.create_idle_process_state(process_state_name="Idle")
    ...
    combined_energy_demand = conveyor_belt_electricity + packaging_machine_electricity + cutting_machine_electricity
    ...
    # Add storages
    cutting_machine.process_state_handler.process_step_data.main_mass_balance.create_storage(current_storage_level=0)
    ...
    packaging_machine.process_state_handler.process_step_data.main_mass_balance.create_storage(current_storage_level=0)
