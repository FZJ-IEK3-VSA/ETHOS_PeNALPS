...
from test.tests.post_processing.load_profile_generator_for_tests import (
...
class TestResamplingAntMatrixConversion(BaseLoadProfileGeneratorClass):
    ...
    def test_load_profile_post_processing(
        ...
        with warnings.catch_warnings():
            ...
            load_profile_meta_data = load_profile_entry_process_process.create_load_profile_meta_data(
            ...
            load_profile_meta_data_compressed = (
                load_profile_entry_process_process._compress_power_in_meta_data_if_necessary(
                    list_of_load_profile_meta_data=load_profile_meta_data
                )
            )
            ...
            meta_data_total_energy_quantity = load_profile_meta_data_compressed.total_energy * Units.get_unit(
                unit_string=load_profile_meta_data_compressed.energy_unit
            )
            # Resample meta data
            load_profile_meta_data_resampled = load_profile_entry_process_process.resample_load_profile_meta_data(
            ...
            total_energy_quantity_resampled = load_profile_meta_data_resampled.total_energy * Units.get_unit(
                unit_string=load_profile_meta_data_resampled.energy_unit
            )
            total_energy_quantity_resampled_converted = total_energy_quantity_resampled.to(
                meta_data_total_energy_quantity.units
            )
            assert type(load_profile_meta_data_resampled) is LoadProfileMetaDataResampled
            assert math.isclose(
            ...
            total_energy_from_power_matrix = carpet_plot_load_profile_generator.get_energy_amount_from_power_matrix(
                carpet_plot_matrix=load_profile_matrix,
            )
            total_energy_power_matrix_quantity = total_energy_from_power_matrix * Units.get_unit(
                unit_string=load_profile_matrix.energy_unit
            )
            total_energy_power_matrix_quantity_converted = total_energy_power_matrix_quantity.to(
                meta_data_total_energy_quantity.u
            )
