...
class InitializationDataCollector:
    ...
    def __init__(self) -> None:
        ...
        self.current_storage_level: numbers_alias
    ...
    def add_current_output_stream_state(self, current_output_stream_state: ContinuousStreamState | BatchStreamState):
    ...
    def add_current_storage_level(self, current_storage_level: numbers_alias):
        """Add the first storage level.

        Args:
            current_storage_level (numbers_alias): The first storage level in the simulation
                data.
        """
...
class ProcessStateNetworkContainer:
    ...
    def __init__(self) -> None:
        self.state_data: CurrentProductionStateData | UninitializedCurrentStateData = UninitializedCurrentStateData()
        self.initialization_data_collector = InitializationDataCollector()

    def update_storage_level(self, new_storage_level: numbers_alias):
        """Updates the storage level in the simulation data.

        Args:
            new_storage_level (numbers_alias): New storage level.

        """
        if new_storage_level < -1:
            warnings.warn("Storage level went below zero at level: " + str(new_storage_level) + "Results are faulty")
        if type(self.state_data) is PostProductionStateData:
            ...
            self.state_data = PostProductionStateData(
                ...
                process_state_data_dictionary=dict(previous_state_data.process_state_data_dictionary),
            )
            logger.debug("Storage level is updated to %s", self.state_data.current_storage_level)
    ...
    def get_storage_level(self) -> numbers_alias:
        """Returns the current storage level.

        Returns:
            numbers_alias: Current storage level
        """
        return self.state_data.current_storage_level

    def update_existing_output_stream_state(self, new_output_stream_state: ContinuousStreamState | BatchStreamState):
    ...
    def add_input_stream_to_validated_data(self, new_input_stream_state: ContinuousStreamState | BatchStreamState):
        ...
        if type(previous_production_state_data) is ValidatedPostProductionStateData:
            self.state_data = PostProductionStateData(
                ...
                validated_input_stream_list=list(previous_production_state_data.validated_input_stream_list),
                current_storage_level=previous_production_state_data.current_storage_level,
                process_state_data_dictionary=dict(previous_production_state_data.process_state_data_dictionary),
            )
            if 1 == 2:
    ...
    def add_first_input_stream_state(self, first_input_stream_state: ContinuousStreamState | BatchStreamState):
    ...
    def adapt_existing_input_stream_state(self, new_input_stream_state: BatchStreamState | ContinuousStreamState):
