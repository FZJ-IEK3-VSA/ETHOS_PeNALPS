...
class BaseMainBalance:
    ...
    def __init__(
        ...
        Args:
            ...
            time_data (TimeData): Store the time data that defines the state of
                the ProcessStep
            input_to_output_conversion_factor (numbers_alias): The conversion
    ...
    def create_storage(
        self,
        current_storage_level: numbers_alias = 0,
        minimum_storage_level_at_start_time_of_production_branch: numbers_alias = 0,
    ...
    def get_missing_mass_for_output_stream_considering_validated_input_streams(
        self,
    ) -> numbers_alias:
        state_data = self.state_data_container.get_validated_pre_or_post_production_state()
        output_stream = self.stream_handler.get_stream(stream_name=state_data.current_output_stream_state.name)
        output_mass = output_stream.get_produced_amount(state=state_data.current_output_stream_state)

        if type(state_data) is (PreProductionStateData):
            validated_input_mass = 0
        else:
            input_stream = self.stream_handler.get_stream(stream_name=state_data.validated_input_stream_list[0].name)
            validated_input_mass = 0
            for input_stream_state in state_data.validated_input_stream_list:
                validated_input_mass = validated_input_mass + input_stream.get_produced_amount(input_stream_state)
    ...
    def get_output_stream_mass_without_storage(self) -> numbers_alias:
        state_data = self.state_data_container.get_validated_pre_or_post_production_state()
        ...
        output_mass_output_commodity = output_stream.get_produced_amount(state=output_stream_state)
        return output_mass_output_commodity


class BranchCompletenessChecker(BaseMainBalance):
    def check_if_production_branch_is_fulfilled(self):
        missing_mass = self.get_missing_mass_for_output_stream_considering_validated_input_streams()
        if missing_mass > 0:
            is_fulfilled = False
    ...
    def check_if_production_branch_is_fulfilled_with_over_production(
        ...
        if self.storage.maximum_storage_level_at_start_time_of_production_branch is None:
            raise Exception(
        ...
        available_mass_in_storage = (
            current_storage_level - self.storage.minimum_storage_level_at_start_time_of_production_branch
        )
        threshold_to_maximum_storage_level = (
            maximum_storage_level_at_start_time_of_production_branch - current_storage_level
        )
        if 0 >= threshold_to_maximum_storage_level:
            input_is_bigger_than_output = self.determine_if_input_stream_is_bigger_than_output()
            ...
            0 <= available_mass_in_storage and 0 <= threshold_to_maximum_storage_level
            ...
            raise Exception("Storage level has an Unexpected case: " + str(current_storage_level))
        return is_fulfilled
    ...
    def check_if_output_stream_can_be_supplied_directly_from_storage(self):
        ...
        available_mass_in_storage_for_provision = (
            state_data.current_storage_level - self.storage.minimum_storage_level_at_start_time_of_production_branch
        )
    ...
    def determine_if_input_stream_is_bigger_than_output(self) -> bool:
        input_stream = self.stream_handler.get_stream(stream_name=self.main_input_stream_name)
        output_stream = self.stream_handler.get_stream(stream_name=self.main_output_stream_name)
        ...
        if input_max_batch_mass > output_max_batch_mass:
            ...
            raise Exception("Request full batch size has been used on symmetric batch size")
...
class MassBalance(BranchCompletenessChecker):
    ...
    def set_input_continuous_stream_parallel_to_output_continuous_stream_with_storage(
        self,
    ) -> ContinuousStreamState:
        ...
        state_data = self.state_data_container.get_validated_pre_or_post_production_state()
        output_stream_state = state_data.current_output_stream_state
        output_stream_mass_output_commodity = self.get_output_stream_mass_without_storage()
        input_stream = self.stream_handler.get_stream(stream_name=self.main_input_stream_name)
        required_input_stream_rate = self.determine_required_input_rate(
            output_stream_state=output_stream_state,
            input_to_output_conversion_factor=self.input_to_output_conversion_factor,
        )
        required_rate_can_be_set = input_stream.check_if_operation_rate_is_within_boundaries(
        ...
        if isinstance(input_stream, ContinuousStream):
            if required_rate_can_be_set is True:
                ...
                logger.debug("A storage is required to provide sufficient input material")
                input_stream_state = input_stream.create_stream_state_for_commodity_amount(
                    commodity_amount=output_stream_mass_output_commodity,
                    end_time=next_stream_end_time,
                )
        else:
            raise UnexpectedDataType(current_data_type=input_stream, expected_data_type=ContinuousStream)
    ...
    def set_input_batch_stream_parallel_to_continuous_output_stream_with_storage(
        self,
    ) -> BatchStreamState:
        ...
        output_stream_mass_output_commodity = (
            self.get_missing_mass_for_output_stream_considering_validated_input_streams()
        )
        ...
        if isinstance(input_stream, BatchStream):
            ...
            raise UnexpectedDataType(current_data_type=input_stream, expected_data_type=BatchStream)
        return input_stream_state
    ...
    def get_output_stream_mass_without_storage(self) -> numbers_alias:
        ...
        Returns:
            numbers_alias: Mass of the output stream state.
        """
        state_data = self.state_data_container.get_validated_pre_or_post_production_state()
        output_stream_state = state_data.current_output_stream_state
        output_stream = self.stream_handler.get_stream(stream_name=self.main_output_stream_name)
    ...
    def set_continuous_input_stream_according_to_output_stream_with_storage(
        ...
        Args:
            operation_rate (numbers_alias, optional): The mass
        ...
        # )
        output_mass_output_commodity = self.get_missing_mass_for_output_stream_considering_validated_input_streams(
            # include_output_stream_mass_in_balance=validated_input_stream_list_is_shorter_than_one
        )
        output_mass_input_commodity = self.convert_output_to_input_mass(output_mass=output_mass_output_commodity)
        input_stream = self.stream_handler.get_stream(stream_name=self.main_input_stream_name)
    ...
    def set_input_continuous_stream_according_to_output_stream_with_storage(
        self,
        operation_rate: numbers_alias = float("inf"),
    ) -> ContinuousStreamState:
        # validated_input_stream_list_is_shorter_than_one = (
    ...
    def set_batch_input_stream_according_to_output_stream_with_storage(
        ...
        output_mass_input_commodity = self.convert_output_to_input_mass(output_mass=output_mass_output_commodity)
        input_stream: BatchStream = self.stream_handler.get_stream(stream_name=self.main_input_stream_name)
        next_stream_end_time = self.time_data.get_next_stream_end_time()
        if isinstance(input_stream, BatchStream):
            possible_batch_mass = input_stream.consider_maximum_batch_mass(
    ...
    def determine_next_stream_end_time_from_previous_input_streams(
        ...
        if type(state_data) is PostProductionStateData:
            ...
            raise Exception("Tried to get the stream start time from an input stream which does not exist")
        else:
    ...
    def determine_required_batch_end_time_to_fulfill_storage(
        ...
        missing_output_mass_output_commodity = self.storage.determine_missing_input_mass(
        ...
        output_stream = self.stream_handler.get_stream(stream_name=self.main_output_stream_name)
        if type(input_stream) is BatchStream:
            possible_batch_mass_input_commodity = input_stream.consider_maximum_batch_mass(
                target_batch_mass=missing_output_mass_input_commodity
            )
        else:
            raise Exception(
                "This method should only be called from process step with an continuous stream as input stream"
            )
        if type(output_stream) is ContinuousStream:
            state_data = self.state_data_container.get_validated_pre_or_post_production_state()
            if type(state_data) is PreProductionStateData:
                last_input_stream_end_time = state_data.current_output_stream_state.end_time
            elif type(state_data) in (
                PostProductionStateData,
                ValidatedPostProductionStateData,
            ):
                last_input_stream_end_time = self.determine_next_stream_end_time_from_previous_input_streams()
                last_input_stream_start_time = self.determine_next_stream_start_time_from_previous_input_streams()
            else:
            ...
            last_part_of_output_stream_state = output_stream.create_stream_state_for_commodity_amount(
    ...
    def convert_output_to_input_mass(self, output_mass: numbers_alias) -> numbers_alias:
        ...
        Args:
            output_mass (numbers_alias): Output mass that should be converted.

        Returns:
            numbers_alias: Input mass value.
        """
        input_mass = output_mass / self.input_to_output_conversion_factor
        return input_mass
    ...
    def convert_input_to_output_mass(self, input_mass: numbers_alias) -> numbers_alias:
        ...
        Args:
            input_mass (numbers_alias): Input mass that should be converted.

        Returns:
            numbers_alias: Output mass value.
        """
        output_mass = input_mass * self.input_to_output_conversion_factor
        return output_mass
    ...
    def determine_required_input_rate(
        ...
        input_stream_operation_rate = output_stream_state.current_operation_rate / input_to_output_conversion_factor
    ...
    def reevaluate_storage_level_according_to_adapted_input_stream(
        ...
        """
        state_data = self.state_data_container.get_validated_or_post_production_state_data()
    ...
    def create_storage(
        ...
        Args:
            current_storage_level (numbers_alias, optional): The storage level at the start of the simulation
                . Defaults to 0.
            minimum_storage_level_at_start_time_of_production_branch (numbers_alias, optional): This is an
                ...
                . Defaults to 0.
            maximum_storage_level_at_start_time_of_production_branch (numbers_alias | None, optional): This is an
                experimental attribute that requires further development. It should provide the capability
    ...
    def check_input_and_output_stream_mass_balance(self):
        ...
        if (
            ...
            and missing_net_stream_mass > -self.acceptable_error_limit_for_stream_balance
        ):
            pass
        else:
            raise Exception(
    ...
    def determine_missing_mass_for_output_stream(self) -> numbers_alias:
