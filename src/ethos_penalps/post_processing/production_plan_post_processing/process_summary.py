...
class ProcessOverViewGenerator:
    def __init__(
        ...
        self.load_type_dict_for_product_specific_energy_demand: dict[LoadType, FinalProductEnergyData] = {}
        ...
        self.stream_specific_energy_demand_product_corrected: dict[str, dict[LoadType, float]] = {}
    ...
    def determine_total_energy_demand_per_ton_of_end_product(self):
        ...
        for stream_name in self.load_profile_handler.stream_energy_data_collection.stream_energy_data_dict:
            stream_to_product_energy_conversion_factor = self.stream_mass_to_product_mass_dict[stream_name]
        ...
        for process_step_name in self.load_profile_handler.process_step_energy_data_handler_dict:
            self.add_process_step_energy_data_to_product_energy_dict(process_step_name=process_step_name)
    ...
    def determine_stream_mass_to_product_mass_conversion_factors(self):
        ...
        self.stream_mass_to_product_mass_dict[current_stream_name] = current_conversion_factor
        ...
        while isinstance(current_node, ProcessStep):
            current_conversion_factor = (
                current_node.process_state_handler.process_step_data.main_mass_balance.convert_output_to_input_mass(
                    output_mass=1
                )
            )
            current_stream_name = (
            ...
            self.stream_mass_to_product_mass_dict[current_stream_name] = previous_stream_to_product_conversion_factor
    ...
    def add_stream_energy_data_to_product_energy_dict(
        ...
        for (
            load_type,
            stream_energy_data,
        ) in self.load_profile_handler.stream_energy_data_collection.stream_energy_data_dict[stream_name].items():
            specific_energy_value_to_add = (
                product_to_stream_conversion_factor * stream_energy_data.specific_energy_demand
            )
    ...
    def add_stream_specific_energy_data(self, stream_name: str, load_type: LoadType, specific_energy_demand: float):
        if stream_name in self.stream_specific_energy_demand_product_corrected:
            if load_type in self.stream_specific_energy_demand_product_corrected[stream_name]:
                raise Exception("Load is already in dict")
            self.stream_specific_energy_demand_product_corrected[stream_name][load_type] = specific_energy_demand
        else:
            self.stream_specific_energy_demand_product_corrected[stream_name] = {load_type: specific_energy_demand}

    def add_process_step_energy_data_to_product_energy_dict(self, process_step_name: str):
        process_step_energy_data_collection = self.load_profile_handler.get_process_step_energy_data_collection(
            process_step_name=process_step_name
        )
        for process_state_energy_data_dict in process_step_energy_data_collection.process_state_energy_dict.values():
            for (
                load_type,
                ...
                if isinstance(
                    ...
                    product_specific_energy_demand = (
                        process_state_energy_data.specific_energy_demand
                        * self.stream_mass_to_product_mass_dict[process_state_energy_data.stream_name]
                    )
                    self.update_product_load_type_energy_data(
                        load_type=load_type,
    ...
    def update_product_load_type_energy_data(
        ...
        if load_type in self.load_type_dict_for_product_specific_energy_demand:
            product_energy_data_type = self.load_type_dict_for_product_specific_energy_demand[load_type]
            product_energy_data_type.specific_energy_demand = (
                product_energy_data_type.specific_energy_demand + energy_value_to_add
            )
            self.load_type_dict_for_product_specific_energy_demand[load_type] = product_energy_data_type
    ...
    def create_empty_product_energy_dict(self):
        ...
        for load_type in list_of_load_types:
            product_energy_data = FinalProductEnergyData(
                specific_energy_demand=0,
                product_commodity=product_commodity,
                load_type=load_type,
            )
            self.load_type_dict_for_product_specific_energy_demand[load_type] = product_energy_data
    ...
    def get_total_target_mass(self) -> float:
        ...
        for order in self.order_dictionary.values():
            total_produced_product_mass = order.production_target + total_produced_product_mass
    ...
    def calculate_total_energy_demands(self) -> pandas.DataFrame:
        ...
        for (
            ...
            total_specific_energy = total_specific_energy + product_energy_data.specific_energy_demand
            total_load_specific_energy_demand = product_energy_data.specific_energy_demand * total_mass
    ...
    def create_product_energy_pie_chart(self) -> matplotlib.pyplot.Figure | None:
        ...
        if self.load_type_dict_for_product_specific_energy_demand:
            total_product_mass_specific_energy_demand_quantity = Units.compress_quantity(
    ...
    def get_total_mass_for_each_stream(self) -> pandas.DataFrame:
        ...
        for stream_data_frame_meta_data in self.production_plan.dict_of_stream_meta_data_data_frames.values():
            # Get the total mass in dependence of the stream
            if stream_data_frame_meta_data.stream_type == BatchStream.stream_type:
                total_mass_produced = stream_data_frame_meta_data.data_frame["batch_mass_value"].sum()
            elif stream_data_frame_meta_data.stream_type == ContinuousStream.stream_type:
                total_mass_produced = stream_data_frame_meta_data.data_frame["total_mass"].sum()
            else:
            ...
            if stream_data_frame_meta_data.stream_name in self.stream_specific_energy_demand_product_corrected:
                stream_energy_data_dict = (
                ...
                for load_type in self.stream_specific_energy_demand_product_corrected[
                    ...
                    specific_energy_demand = self.stream_specific_energy_demand_product_corrected[
                        stream_data_frame_meta_data.stream_name
                    ][load_type]
                    ...
                    column_name_specific_energy_demand = "Specific energy demand " + str(load_type.name)
                    energy_demand_row_dictionary[column_name_specific_energy_demand] = specific_energy_demand
                    ...
                    ### Value
                    total_energy_demand_column_name = "Total energy demand " + str(load_type.name)
                    ...
                    energy_demand_row_dictionary[total_energy_demand_column_name] = total_energy_demand_quantity.m
                    energy_demand_row_dictionary["Total energy unit"] = total_energy_demand_quantity.u
                ...
                for (
                    ...
                    for (
                        ...
                        if isinstance(
                            ...
                            if process_step_name in process_state_energy_dict_by_process_step_name:
                                if load_type in process_state_energy_dict_by_process_step_name[process_step_name]:
                                    total_energy = process_state_energy_dict_by_process_step_name[process_step_name][
                                        load_type
                                    ]["energy_quantity"].sum()
                                    ...
                                    mass_throughput = total_energy / product_specific_energy_demand

                                    energy_row_dictionary = {
                                        "Total energy: " + str(load_type.name): total_energy_demand_quantity.value,
