...
class StreamPostProcessor:
    def __init__(
        ...
        list_of_stream_states: list[ContinuousStreamProductionPlanEntry | BatchStreamProductionPlanEntry],
    ) -> None:
        self.stream: ContinuousStream | BatchStream = stream
        self.list_of_stream_states: list[ContinuousStreamProductionPlanEntry | BatchStreamProductionPlanEntry] = (
    ...
    def determine_total_mass_in_time(
        ...
        stream_data_frame = dataframe_from_dataclasses(self.list_of_stream_states)
    ...
    def determine_actual_throughput(
        ...
        if isinstance(self.stream, ContinuousStream):
            ...
            total_stream_mass = stream_data_frame["total_mass"].sum()
        time_range_number = (earliest_start_date - latest_end_date) / datetime.timedelta(hours=1)
    ...
    def fill_from_date_to_start(
        ...
        if check_if_date_1_is_before_date_2(date_1=start_date, date_2=first_entry.start_time):
            if isinstance(self.stream, ContinuousStream):
                ...
                production_plan_entry = self.stream.create_production_plan_entry(state=stream_state)
            elif isinstance(self.stream, BatchStream):
    ...
    def fill_to_end_date(
        self,
        list_of_stream_entries: list[ContinuousStreamProductionPlanEntry | BatchStreamProductionPlanEntry],
...
class ProcessStepPostProcessor:
    def __init__(
        ...
        self.list_of_process_step_entries: list[ProcessStepProductionPlanEntry] = list_of_process_step_entries[::-1]
        self.process_step: ProcessStep = process_step
        self.input_stream_post_processor: StreamPostProcessor = input_stream_post_processor
        self.output_stream_post_processor: StreamPostProcessor = output_stream_post_processor
    ...
    def fill_from_date_to_start(
        ...
        if check_if_date_1_is_before_date_2(date_1=start_date, date_2=first_entry.start_time):
            ...
            new_first_process_step_entry = idle_state._create_process_step_production_plan_entry(
    ...
    def fill_to_end_date(
        ...
        if check_if_date_1_is_before_date_2(date_1=last_entry.end_time, date_2=end_date):
            idle_state = self.process_step.process_state_handler.get_idle_state()
            new_last_process_step_entry = idle_state._create_process_step_production_plan_entry(
                process_state_state=ProcessStateData(
                    process_state_name=idle_state.process_state_name,
                    start_time=last_entry.end_time,
                    end_time=end_date,
    ...
    def get_earliest_start_date_process_step(self) -> datetime.datetime:
        # Attention Process Step list and Stream list are temporally inverted
        return self.list_of_process_step_entries[0].start_time

    def get_latest_end_date_process_step(self) -> datetime.datetime:
        # Attention Process Step list and Stream list are temporally inverted
        return self.list_of_process_step_entries[-1].end_time

    def get_earliest_start_date_input_stream(self) -> datetime.datetime:
        # Attention Process Step list and Stream list are temporally inverted
        return self.input_stream_post_processor.list_of_stream_states[-1].start_time

    def get_latest_end_date_input_stream(self) -> datetime.datetime:
        # Attention Process Step list and Stream list are temporally inverted
        return self.input_stream_post_processor.list_of_stream_states[0].end_time

    def get_earliest_start_date_output_stream(self) -> datetime.datetime:
        # Attention Process Step list and Stream list are temporally inverted
        return self.output_stream_post_processor.list_of_stream_states[-1].start_time

    def get_latest_end_date_output_stream(self) -> datetime.datetime:
        # Attention Process Step list and Stream list are temporally inverted
        return self.output_stream_post_processor.list_of_stream_states[0].end_time
    ...
    def determine_mass_throughput(
        ...
        if earliest_start_date is None:
            earliest_start_date = self.get_earliest_start_date_process_step()
        if latest_end_date is None:
            latest_end_date = self.get_latest_end_date_process_step()
        total_mass_in_time = (
        ...
        total_hours = (latest_end_date - earliest_start_date) / datetime.timedelta(hours=1) * Units.get_unit("h")
        mass_stream = total_mass_in_time / total_hours
        if pretty_print is True:
            output_string = "Process Step: " + self.process_step.name + " has a throughput of " + str(mass_stream)
            print(output_string)
        return mass_stream

    def determine_mass_throughput_based_on_stream(
        self,
        earliest_start_date: datetime.datetime | None = None,
        latest_end_date: datetime.datetime | None = None,
        pretty_print: bool = False,
        based_on_input_stream: bool = True,
    ) -> pint.Quantity:
        if earliest_start_date is None:
            if based_on_input_stream is True:
                earliest_start_date = self.get_earliest_start_date_input_stream()
            if based_on_input_stream is False:
                earliest_start_date = self.get_earliest_start_date_output_stream()
        if latest_end_date is None:
            if based_on_input_stream is True:
                latest_end_date = self.get_latest_end_date_input_stream()
            if based_on_input_stream is False:
                latest_end_date = self.get_latest_end_date_output_stream()
        total_mass_in_time = (
            self.output_stream_post_processor.determine_total_mass_in_time(
                earliest_start_date=earliest_start_date, latest_end_date=latest_end_date
            )
...
class ProductionPlanPostProcessor:
    ...
    def create_all_process_step_processors(self) -> dict[str, ProcessStepPostProcessor]:
        ...
        for process_step_name in self.production_plan.process_step_states_dict:
            dict_of_process_step_processors[process_step_name] = self.create_process_step_processor(
    ...
    def determine_throughput_for_process_step(
        ...
        post_load_profile_processor_process_step = post_production_post_processor_dict[process_step_name]
        current_throughput = post_load_profile_processor_process_step.determine_mass_throughput()
    ...
    def determine_idle_time_for_process_step(
        ...
        if start_date is not None:
            ...
            post_load_profile_processor_process_step.list_of_process_step_entries = list_of_process_step_entries
    ...
    def create_process_step_processor(self, process_step_name: str) -> ProcessStepPostProcessor:
        process_step = self.process_node_dict[process_step_name]
        input_stream_name = process_step.get_input_stream_name()
        output_stream_name = process_step.get_output_stream_name()

        input_stream_post_processor = self.create_stream_post_processor(stream_name=input_stream_name)
        output_stream_post_processor = self.create_stream_post_processor(stream_name=output_stream_name)
        list_of_process_step_states = self.production_plan.process_step_states_dict[process_step_name]
    ...
    def create_stream_post_processor(self, stream_name: str) -> StreamPostProcessor:
        ...
        stream_post_processor = StreamPostProcessor(stream=stream, list_of_stream_states=list_of_stream_states)
    ...
    def determine_earliest_process_state(self) -> datetime.datetime:
        ...
        for process_step_processor in dict_of_process_step_processors.values():
            start_date = process_step_processor.get_earliest_start_date_process_step()
    ...
    def determine_latest_end_time(self) -> datetime.datetime:
        ...
        list_of_end_dates = []
        for process_step_processor in dict_of_process_step_processors.values():
            end_date = process_step_processor.get_earliest_start_date_process_step()
