...
class NetworkAnalyzer:
    ...
    def get_list_of_all_stream_process_step_names(self) -> list[str]:
        ...
        Returns:
            list[str]: List of process step names.
        """
        list_of_object_names = []
        for network_level in self.list_of_network_level:
            list_of_process_step_node_names = network_level.get_list_of_process_step_node_names()
    ...
    def get_dictionary_of_nodes_names_keyed_by_chain_names(
        ...
        for network_level in self.list_of_network_level:
            for process_chain in network_level.list_of_process_chains:
                ...
                object_name_dictionary[process_chain.process_chain_identifier.chain_name] = list_of_nodes
                include_sink = False

        return object_name_dictionary

    def get_downstream_stream_name(self, process_chain: ProcessChain, process_step_name: str) -> str:
        process_step = process_chain.get_process_node(process_node_name=process_step_name)
    ...
    def get_upstream_stream_name(self, process_chain: ProcessChain, process_step_name: str) -> str:
        process_step = process_chain.get_process_node(process_node_name=process_step_name)
        if type(process_step) is ProcessStep:
            input_stream_name = (
    ...
    def get_source_name_from_network_level(self, network_level: NetworkLevel) -> str:
        return network_level.get_main_source().name

    def get_sink_name_from_network_level(self, network_level: NetworkLevel) -> str:
        return network_level.get_main_sink().name

    def get_sink_network_level(self) -> NetworkLevel:
        return self.list_of_network_level[0]


class NetworkEnergyAnalyzer(NetworkAnalyzer):
    def determine_specific_energy_demand_of_process_chain(self, process_chain: ProcessChain):
        sink = process_chain.get_sink()
        input_stream = sink.get_input_stream_name()

    def convert_stream_energy_data_to_output_mass(
        self, conversion_factor: float, specific_energy_demand: float
    ) -> float:
        output_specific_energy_demand = specific_energy_demand / conversion_factor
        return output_specific_energy_demand

    def determine_energy_demand_per_product_mass(self):
        load_profile_handler_simulation = LoadProfileHandlerSimulation()
        list_of_load_profile_entries = load_profile_handler_simulation.get_list_of_load_types()
        dict_of_specific_energy_data_per_load_type = {}


@dataclass
class StreamResults:
    """Contains all simulation results about a BatchStream or ContinuousStream."""

    stream_meta_data_frame: StreamDataFrameMetaInformation
    list_of_load_profile_meta_data: list[LoadProfileMetaData]
    list_of_resampled_load_profile_meta_data: list[LoadProfileMetaDataResampled]
    ...
    ) -> list[StreamDataFrameMetaInformation | LoadProfileMetaData | EmptyMetaDataInformation]:
        output_meta_data_list: list[
    ...
    def get_stream_and_load_profile_resampled_meta_data_list(
        self, load_profiles_above: bool = True
    ) -> list[StreamDataFrameMetaInformation | LoadProfileMetaDataResampled | EmptyMetaDataInformation]:
        output_meta_data_list: list[
            StreamDataFrameMetaInformation | LoadProfileMetaDataResampled | EmptyMetaDataInformation
        ] = []
        if load_profiles_above is True:
            output_meta_data_list.extend(self.list_of_resampled_load_profile_meta_data)
        output_meta_data_list.append(self.stream_meta_data_frame)
        if load_profiles_above is False:
            output_meta_data_list.extend(self.list_of_resampled_load_profile_meta_data)
        return output_meta_data_list

    def get_stream_and_load_profile_resampled_and_not_resampled_meta_data_list(
        self, load_profiles_above: bool = True
    ) -> list[
        StreamDataFrameMetaInformation | LoadProfileMetaData | LoadProfileMetaDataResampled | EmptyMetaDataInformation
    ]:
        output_meta_data_list: list[
            StreamDataFrameMetaInformation
            | LoadProfileMetaData
        ...
        current_iterator = 0
        if load_profiles_above is True:
            for current_meta_data in self.list_of_load_profile_meta_data:
                current_resampled_meta_data = self.list_of_resampled_load_profile_meta_data[current_iterator]
                output_meta_data_list.append(current_meta_data)
                output_meta_data_list.append(current_resampled_meta_data)
    ...
    def return_list_of_stream_stream_results(
        self,
        include_non_sampled_load_profiles: bool,
        include_sampled_load_profiles: bool,
    ) -> list[
        StreamDataFrameMetaInformation | EmptyMetaDataInformation | LoadProfileMetaData | LoadProfileMetaDataResampled
    ]:
        list_of_load_profile_meta_data: list[
            StreamDataFrameMetaInformation
            | EmptyMetaDataInformation
            | LoadProfileMetaData
            | LoadProfileMetaDataResampled
        ] = []
        if include_non_sampled_load_profiles is False and include_sampled_load_profiles is False:
            list_of_load_profile_meta_data.extend(self.get_stream_meta_data_list())
        elif include_non_sampled_load_profiles is True and include_sampled_load_profiles is False:
            list_of_load_profile_meta_data.extend(self.get_stream_and_load_profile_meta_data_list())
        elif include_non_sampled_load_profiles is False and include_sampled_load_profiles is True:
            list_of_load_profile_meta_data.extend(self.get_stream_and_load_profile_resampled_meta_data_list())
        elif include_non_sampled_load_profiles is True and include_sampled_load_profiles is True:
            list_of_load_profile_meta_data.extend(
                self.get_stream_and_load_profile_resampled_and_not_resampled_meta_data_list()
            )
...
class ProcessStepResults:
    """Contains alls structured results
    ...
    list_load_profile_meta_data_frame: list[LoadProfileMetaData]
    list_load_profile_resampled_meta_data_frame: list[LoadProfileMetaDataResampled]

    def get_meta_data_list_with_input_and_output_streams(
        self,
        include_non_sampled_load_profiles: bool,
        include_sampled_load_profiles: bool,
        input_at_top: bool = True,
        include_internal_storage: bool = False,
    ) -> list[
        ProcessStepDataFrameMetaInformation
        | StreamDataFrameMetaInformation
        | LoadProfileMetaData
        ...
        for stream_result in upper_stream_list:
            list_of_load_profile_meta_data = stream_result.return_list_of_stream_stream_results(
                include_non_sampled_load_profiles=include_non_sampled_load_profiles,
                include_sampled_load_profiles=include_sampled_load_profiles,
            )
            output_meta_data_list.extend(list_of_load_profile_meta_data)

        if include_internal_storage is True:
            output_meta_data_list.append(self.internal_storage_meta_data)

        list_of_process_step_load_profile_meta_data = self.select_process_step_load_profiles(
            include_non_sampled_load_profiles=include_non_sampled_load_profiles,
            include_sampled_load_profiles=include_sampled_load_profiles,
        )
        output_meta_data_list.extend(list_of_process_step_load_profile_meta_data)
    ...
    def get_meta_data_list_and_input_streams(
        ...
        include_sampled_load_profiles: bool,
        input_at_top: bool = True,
        include_internal_storages: bool = False,
    ) -> list[
        ProcessStepDataFrameMetaInformation
        | StreamDataFrameMetaInformation
        | StorageDataFrameMetaInformation
    ...
    def get_meta_data_list_with_output_streams(
        self,
        include_non_sampled_load_profiles: bool,
        include_sampled_load_profiles: bool,
        output_at_top: bool = True,
    ) -> list[
        ProcessStepDataFrameMetaInformation
        | StreamDataFrameMetaInformation
        | StorageDataFrameMetaInformation
    ...
    def select_process_step_load_profiles(
        self,
        include_non_sampled_load_profiles: bool,
        include_sampled_load_profiles: bool,
    ) -> list[LoadProfileMetaData | LoadProfileMetaDataResampled]:
        list_of_process_step_load_profile_meta_data: list[LoadProfileMetaData | LoadProfileMetaDataResampled] = []
        if include_non_sampled_load_profiles is False and include_sampled_load_profiles is False:
            pass
        elif include_non_sampled_load_profiles is True and include_sampled_load_profiles is False:
            list_of_process_step_load_profile_meta_data.extend(self.list_load_profile_meta_data_frame)
        elif include_non_sampled_load_profiles is False and include_sampled_load_profiles is True:
            list_of_process_step_load_profile_meta_data.extend(self.list_load_profile_resampled_meta_data_frame)

        elif include_non_sampled_load_profiles is True and include_sampled_load_profiles is True:
            current_iterator = 0

            for current_meta_data in self.list_load_profile_meta_data_frame:
                current_resampled_meta_data = self.list_load_profile_resampled_meta_data_frame[current_iterator]
                list_of_process_step_load_profile_meta_data.append(current_meta_data)
                list_of_process_step_load_profile_meta_data.append(current_resampled_meta_data)
                current_iterator = current_iterator + 1

        return list_of_process_step_load_profile_meta_data


@dataclass
class SinkResults:
    name: str
    storage_meta_data_frame: StorageDataFrameMetaInformation | EmptyMetaDataInformation
    list_of_input_stream_results: list[StreamResults]
    ...
    def get_streams_and_storage_meta_data(
        self,
        include_order_meta_data: bool = True,
        include_storage_meta_data: bool = True,
    ) -> list[StorageDataFrameMetaInformation | StreamDataFrameMetaInformation | ProductionOrderMetadata]:
        ...
        for input_stream_result in self.list_of_input_stream_results:
            ...
            output_meta_data_list.extend(input_stream_result.get_stream_meta_data_list())

        if include_order_meta_data is True:
            output_meta_data_list.append(self.total_order_collection_metadata)
        if include_storage_meta_data is True:
            output_meta_data_list.append(self.storage_meta_data_frame)

        return output_meta_data_list

    def _get_splitted_order_meta_data(self, stream_name: str) -> ProductionOrderMetadata:
        splitted_order_meta_data = self.dict_of_splitted_order_meta_data_frame[stream_name]
...
class ProcessChainStorageResults:
    ...
    list_of_output_stream_results: list[StreamResults]
    order_collection: OrderCollection
    dict_of_splitted_order_collection: dict[ProcessChainIdentifier, SplittedOrderCollection]
    total_order_collection_metadata: ProductionOrderMetadata
    dict_of_splitted_order_meta_data_frame: dict[str, ProductionOrderMetadata]
    ...
    def get_streams_and_storage_meta_data(
        ...
        for output_stream_result in self.list_of_output_stream_results:
            output_meta_data_list.extend(output_stream_result.get_stream_meta_data_list())
        return output_meta_data_list
...
class SourceResults:
    ...
    def get_streams_and_storage_meta_data(
        self, include_order_meta_data: bool = True, include_storage_meta_data=True
    ) -> list[StorageDataFrameMetaInformation | StreamDataFrameMetaInformation | EmptyMetaDataInformation]:
        output_meta_data_list = []
        if include_storage_meta_data is True:
            output_meta_data_list.append(self.storage_meta_data_frame)
        for output_stream_result in self.list_of_output_stream_results:
...
class ProcessChainResults:
    ...
    def get_process_chain_without_sources_and_sinks(
        self,
        include_internal_storages: bool,
        include_load_profiles: bool,
        include_load_profiles_resampled: bool,
    ) -> list[
        StreamDataFrameMetaInformation
        | ProcessStepDataFrameMetaInformation
        | LoadProfileMetaData
        ...
        if self.upstream_end_node_position > self.downstream_end_node_position:
            self.list_of_process_step_results = list(reversed(self.list_of_process_step_results))
            ...
            if node_position_counter == self.downstream_end_node_position:
                output_meta_data_list.extend(
                    process_step_results.get_meta_data_list_with_input_and_output_streams(
                        ...
                        include_sampled_load_profiles=include_load_profiles,
                        include_non_sampled_load_profiles=include_load_profiles_resampled,
                    )
                )
            else:
                output_meta_data_list.extend(
                    process_step_results.get_meta_data_list_and_input_streams(
                        ...
                        include_non_sampled_load_profiles=include_load_profiles,
                        include_sampled_load_profiles=include_load_profiles_resampled,
                    )
                )
...
class StructuredNetworkResults:
    ...
    def get_network_level_in_material_flow_direction(
        ...
        self.downstream_network_level_position = len(self.list_of_structured_level_results)
    ...
    def get_network_level_in_reversed_material_flow_direction(
        ...
        self.upstream_network_level_position = len(self.list_of_structured_level_results)
...
class ResultSelector:
    ...
    def get_structured_network_results(
        self,
        report_options: ReportGeneratorOptions,
    ) -> StructuredNetworkResults:
        list_of_structured_level_results = []
        include_storage_results: bool = report_options.gantt_charts.include_storage_gantt_charts
        assert self.post_process_simulation_data_handler.postprocessing_is_initialized is True
        for network_level in self.list_of_network_level:
            structured_network_level_results = self._create_structured_network_level_results(
                network_level=network_level,
                include_storage_results=include_storage_results,
            )
    ...
    def _create_structured_network_level_results(
        self, network_level: NetworkLevel, include_storage_results: bool
        ...
        if type(network_level.main_source) is Source:
            source_results = self._get_source_results(
                source=network_level.main_source,
                include_storage_results=include_storage_results,
            )
        ...
        if type(network_level.main_sink) is Sink:
            sink_results = self._get_sink_results(
                sink=network_level.main_sink,
                include_storage_results=include_storage_results,
            )
        ...
        for process_chain in network_level.list_of_process_chains:
            list_of_process_chain_results.append(self._get_process_chain_results(process_chain=process_chain))
    ...
    def _get_process_chain_results(self, process_chain: ProcessChain) -> ProcessChainResults:
        list_of_process_step_results = []
        current_node = process_chain.sink
        current_node.prepare_sink_for_next_chain(process_chain_identifier=process_chain.process_chain_identifier)
        ...
        while isinstance(current_node, ProcessStep):
            list_of_process_step_results.append(self._get_process_step_results(process_step=current_node))
            current_node_name = current_node.get_upstream_node_name()
            current_node = process_chain.process_node_dict[current_node_name]
    ...
    def _get_process_step_results(self, process_step: ProcessStep) -> ProcessStepResults:
        ...
        internal_storage_meta_data_frame = self.post_process_simulation_data_handler.get_storage_meta_data_by_name(
        ...
        list_of_process_step_load_profile_meta_data = (
            self.post_process_simulation_data_handler.get_list_of_process_step_load_profile_meta_data(
                process_step_name=process_step.name
            )
        )
        list_of_process_step_load_profile_meta_data_resampled = (
            self.post_process_simulation_data_handler.get_list_of_process_step_load_profile_meta_data_resampled(
                process_step_name=process_step.name
            )
        )
        process_step_results = ProcessStepResults(
            ...
            list_load_profile_meta_data_frame=list_of_process_step_load_profile_meta_data,
            list_load_profile_resampled_meta_data_frame=list_of_process_step_load_profile_meta_data_resampled,
        )
        return process_step_results

    def _get_sink_results(self, sink: Sink, include_storage_results: bool) -> SinkResults:

        if include_storage_results is True:
            # Get storage meta data frame
            storage_meta_data_frame = self.post_process_simulation_data_handler.get_storage_meta_data_by_name(
                storage_name=sink.name
            )
        else:
            storage_meta_data_frame = EmptyMetaDataInformation(name="source.name", object_type="Storage")
        # Get output stream result list
        list_of_input_stream_results = []
        for input_stream_name in sink.order_distributor.dict_of_stream_names.values():
            input_stream_results = self._get_stream_results(stream_name=input_stream_name)
            list_of_input_stream_results.append(input_stream_results)
        ...
        for (
            ...
            splitted_order = sink.order_distributor.dict_of_splitted_order[process_chain_identifier]
            splitted_order_meta_data = post_process_order_collection(order_collection=splitted_order)
            dict_of_splitted_order_meta_data_by_stream_name[stream_name] = splitted_order_meta_data
    ...
    def _get_process_chain_storage_results(
        self, process_chain_storage: ProcessChainStorage, include_storage_results: bool
        ...
        list_of_input_stream_results = []
        for input_stream_name in process_chain_storage.sink.order_distributor.dict_of_stream_names.values():
            input_stream_results = self._get_stream_results(stream_name=input_stream_name)
            list_of_input_stream_results.append(input_stream_results)

        # Get output stream result list
        list_of_output_stream_results = []
        for output_stream_name in process_chain_storage.source.dict_of_output_stream_names.values():
            output_stream_results = self._get_stream_results(stream_name=output_stream_name)
        ...
        for (
            ...
            splitted_order = process_chain_storage.sink.order_distributor.dict_of_splitted_order[
                process_chain_identifier
            ]
            splitted_order_meta_data = post_process_order_collection(order_collection=splitted_order)
            dict_of_splitted_order_meta_data_by_stream_name[stream_name] = splitted_order_meta_data
    ...
    def _get_source_results(self, source: Source, include_storage_results: bool) -> SourceResults:
        if include_storage_results is True:
            storage_meta_data_frame = self.post_process_simulation_data_handler.get_storage_meta_data_by_name(
    ...
    def _get_stream_results(self, stream_name: str) -> StreamResults:
        stream_meta_data_frame = self.post_process_simulation_data_handler.get_stream_meta_data_by_name(
            stream_name=stream_name
        )
        list_of_stream_load_profile_meta_data = (
            self.post_process_simulation_data_handler.get_list_of_stream_load_profile_meta_data(stream_name=stream_name)
        )
        list_of_stream_load_profile_meta_data_resampled = (
            self.post_process_simulation_data_handler.get_list_of_stream_load_profile_resampled_meta_data(
                stream_name=stream_name
            )
        )
        stream_results = StreamResults(
            stream_meta_data_frame=stream_meta_data_frame,
            list_of_load_profile_meta_data=list_of_stream_load_profile_meta_data,
            list_of_resampled_load_profile_meta_data=list_of_stream_load_profile_meta_data_resampled,
        )
