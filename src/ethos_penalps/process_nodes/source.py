...
from ethos_penalps.data_classes import (
    ...
    ProductionOrder,
    ProductionOrderWithTransferDuration,
    StaticTimePeriod,
...
class Source(ProcessNode):
    ...
    def __init__(
        ...
        self.list_of_output_stream_states: list[ContinuousStreamState | BatchStreamState] = []
    ...
    def add_output_stream(
        ...
        if not isinstance(output_stream, ContinuousStream | BatchStream):
            raise Exception("Expected output stream of type ContinuousStream but got type: " + str(type(output_stream)))

        self.dict_of_output_stream_names[process_chain_identifier] = output_stream.name

    def create_storage_entries(
        self,
        assume_output_streams_as_input: bool = False,
    ) -> list[StorageProductionPlanEntry] | None:
        ...
        if self.storage_entries_have_been_created is False:
            if assume_output_streams_as_input:
                input_list = self.list_of_output_stream_states
                output_list = []
            else:
                input_list = []
                output_list = self.list_of_output_stream_states

            list_of_storage_entries, _, _ = self.storage.create_storage_entries_from_streams(
                input_stream_state_list=input_list,
                output_stream_state_list=output_list,
                storage_level_at_start="auto_offset",
            )

            if assume_output_streams_as_input is False:
            ...
            logger.debug("Storage entries have already been created.")
    ...
    def set_current_output_stream(
        ...
        """
        self.current_output_stream_name = self.dict_of_output_stream_names[process_chain_identifier]
    ...
    def prepare_source_for_next_chain(
        ...
        """
        self.set_current_output_stream(process_chain_identifier=process_chain_identifier)
        self.production_branch_number: float = 0
        self.temporal_branch_number: float = 0
    ...
    def get_downstream_node_name(self) -> str:
        ...
        chain downstream of the source.
        """
        output_stream = self.stream_handler.get_stream(stream_name=self.current_output_stream_name)
    ...
    def create_complete_branch_data(
        ...
        output_branch_identifier = OutputBranchIdentifier(branch_number=self.production_branch_number)
    ...
    def process_input_order(self, input_node_operation: NodeOperation) -> DownstreamValidationOrder:
        ...
        if isinstance(input_node_operation, UpstreamNewProductionOrder):
            ...
            raise Exception("Unexpected node operation " + str(input_node_operation) + " in Source: " + str(self.name))
    ...
    def create_production_order_collection_from_input_states(
        ...
        order_number = 0
        output_stream = self.stream_handler.get_stream(stream_name=self.current_output_stream_name)
        mass_unit = output_stream.static_data.mass_unit
        ...
        for stream_state in reversed(self.list_of_output_stream_states):
            ...
            if isinstance(stream_state, ContinuousStreamState):
                ...
                raise Exception("There are only  + " + str(self.name) + " ")
        ...
        order_data_frame = dataframe_from_dataclasses(list(production_order_dict.values()))
        order_data_frame.sort_values(by="production_deadline", ascending=False, inplace=True)
        order_data_frame.reset_index(inplace=True, drop=True)

        target_mass = order_data_frame.loc[:, ["production_target"]].sum()["production_target"]
    ...
    def create_production_order_with_transfer_duration_collection_from_input_states(
        self,
    ) -> OrderWithMassTransferDurationCollection:
        """Converts the requested output streams into new orders.
        ...
        for stream_state in reversed(self.list_of_output_stream_states):
            ...
            if isinstance(stream_state, ContinuousStreamState):
                production_deadline = stream_state.start_time
                transfer_duration = stream_state.end_time - stream_state.start_time
            elif isinstance(stream_state, BatchStreamState):
                production_deadline = stream_state.start_time
                transfer_duration = stream_state.end_time - stream_state.start_time
            else:
                raise Exception("There are only  + " + str(self.name) + " ")
            production_order = ProductionOrderWithTransferDuration(
                production_target=total_stream_mass,
                production_deadline=production_deadline,
        ...
        target_mass = order_data_frame.loc[:, ["production_target"]].sum()["production_target"]
        order_collection = OrderWithMassTransferDurationCollection(
            target_mass=target_mass,
            commodity=self.commodity,
            order_data_frame=order_data_frame,
        )
    ...
    def create_production_order_collection_order_with_transfer_period(
        self,
    ) -> OrderCollection:
        """Converts the requested output streams into new orders.
