...
# from ethos_penalps.petri_net.process_state import IntermediateStateBasedOnStorage
...
class StreamLoadProfileEntryCollection:
    ...
    load type itself.
    """
    dict_of_load_entry_lists: dict[str, list[LoadProfileEntry]] = field(default_factory=dict)
...
class LoadProfileCollection:
    ...
    dict_stream_load_profile_collections: dict[str, StreamLoadProfileEntryCollection] = field(default_factory=dict)
    """Contains the load profile data for all streams
    The key is the stream name
    """
    # Contains the load profile data for all streams
    # The key is the process step
    dict_process_step_load_profile_collections: dict[str, ProcessStepLoadProfileEntryCollection] = field(
    ...
    def append_stream_load_profile_entry(
        ...
        if stream_name not in self.dict_stream_load_profile_collections:
            stream_load_profile_entry_collection = StreamLoadProfileEntryCollection(object_name=stream_name)
            self.dict_stream_load_profile_collections[stream_name] = stream_load_profile_entry_collection
        else:
            stream_load_profile_entry_collection = self.dict_stream_load_profile_collections[stream_name]
    ...
    def append_process_step_energy_data_entry(
        ...
        """Appends a single load LoadProfileEntry instance from a process
        step.

        Args:
            process_step_name (str): Name of the ProcessStep
            load_type (LoadType): Identifies the load type of the LoadProfileEntry
            load_profile_entry (LoadProfileEntry): Stores the information about the
                energy consumption of ProcessStep für certain time period.
        """
        if process_step_name not in self.dict_process_step_load_profile_collections:
            process_step_lp_collection = self.dict_process_step_load_profile_collections[process_step_name] = (
                ProcessStepLoadProfileEntryCollection(object_name=process_step_name)
            )
        else:
            process_step_lp_collection = self.dict_process_step_load_profile_collections[process_step_name]
        process_step_lp_collection.add_load_profiles(load_type=load_type, load_profile_entry=load_profile_entry)

    def get_load_profile_dictionary(
        self,
    ) -> dict[str, ProcessStepLoadProfileEntryCollection]:
        return self.dict_process_step_load_profile_collections
...
class ProcessStepEnergyDataHandler:
    ...
    process_step_name: str
    process_state_energy_dict: dict[str, ProcessStateEnergyData] = field(default_factory=dict)
    ...
    def get_process_state_energy_data(self, process_state_name: str) -> ProcessStateEnergyData:
...
class StreamSpecificEnergyDataHandler:
    ...
    def get_list_of_load_types(self) -> list[LoadType]:
        list_of_load_types = list(self.dict_of_all_load_types.values())
        return list_of_load_types
...
class LoadProfileHandlerSimulation:
    def __init__(self) -> None:
        ...
        self.process_step_energy_data_handler_dict: dict[str, ProcessStepEnergyDataHandler] = {}
        self.stream_energy_data_collection: StreamSpecificEnergyDataHandler = StreamSpecificEnergyDataHandler()

    def get_list_of_load_types(self) -> list[LoadType]:
        dict_of_load_types = self.stream_energy_data_collection.get_dict_of_load_types()

        for (process_step_energy_data_handler,) in self.process_step_energy_data_handler_dict.values():
            process_step_energy_data_handler: ProcessStepEnergyDataHandler
            current_dict_of_load_types = process_step_energy_data_handler.get_dict_of_load_types()
    ...
    def get_list_of_list_of_all_load_profile_entries(
        ...
        for (
            ...
            for (
                ...
                list_of_list_of_all_load_profile_entries.append(list_of_load_profiles_entries)
    ...
    def create_load_profile_entry_from_stream_entry(
        self,
        stream_entry: (ContinuousStreamProductionPlanEntry | BatchStreamProductionPlanEntry),
        stream_energy_data: StreamLoadEnergyData,
    ) -> LoadProfileEntry:
        if isinstance(stream_entry, ContinuousStreamProductionPlanEntry):
            energy_demand = stream_entry.total_mass * stream_energy_data.specific_energy_demand
        elif isinstance(stream_entry, BatchStreamProductionPlanEntry):
            energy_demand = stream_entry.total_mass * stream_energy_data.specific_energy_demand
        else:
            raise Exception("Unexpected datatype in stream entry")
        average_power_consumption = energy_demand / (stream_entry.end_time - stream_entry.start_time).total_seconds()
        load_profile_entry = LoadProfileEntry(
    ...
    def create_all_load_profiles_entries_from_stream_entry(
        ...
        if stream_entry.name in self.stream_energy_data_collection.stream_energy_data_dict:
            stream_energy_data = self.stream_energy_data_collection.get_stream_energy_data(
                stream_name=stream_entry.name
            )
            for stream_energy_load_data in stream_energy_data.dict_stream_load_energy_data.values():
    ...
    def add_process_state_energy_data(
        ...
        if process_step_name not in self.process_step_energy_data_handler_dict:
            self.process_step_energy_data_handler_dict[process_step_name] = ProcessStepEnergyDataHandler(
                process_step_name=process_step_name
            )
        self.process_step_energy_data_handler_dict[process_step_name].add_process_state_energy_data(
            process_state_name=process_state_name,
            process_state_energy_data=process_state_energy_data,
        )

    def get_process_step_energy_data_collection(self, process_step_name: str) -> ProcessStepEnergyDataHandler:
    ...
    def create_all_load_profiles_from_process_state_entry(
        ...
        if process_state_entry.process_step_name in self.process_step_energy_data_handler_dict:
            energy_data_collection = self.process_step_energy_data_handler_dict[process_state_entry.process_step_name]

            if process_state_entry.process_state_name in energy_data_collection.process_state_energy_dict:
                process_state_energy_data = energy_data_collection.get_process_state_energy_data(
                    process_state_name=process_state_entry.process_state_name,
                )
                for process_state_energy_data in process_state_energy_data.dict_of_load_energy_data.values():
                    if isinstance(
                        ...
                        total_mass = process_state_entry.relevant_mass
                        energy_demand = total_mass * process_state_energy_data.specific_energy_demand

                        average_power_consumption = (
                            energy_demand
                            / (process_state_entry.end_time - process_state_entry.start_time).total_seconds()
                        )
                        load_profile_entry = LoadProfileEntry(
