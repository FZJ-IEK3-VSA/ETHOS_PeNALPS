...
def define_and_start_simulation():
    ...
    time_data = TimeData(global_start_date=start_date, global_end_date=end_date)
    ...
    process_step = process_chain.create_process_step(name="Batch to conti step")
    # Process nodes
    ...
    - Either 
        ...
        or 
            - an input stream reueststing state
            ...
            - and input stream reueststing state
    ...
    input_and_output_stream_providing_state = (
        process_step.process_state_handler.create_state_for_parallel_input_and_output_stream_with_storage(
            process_state_name="Batch to Conti Simultaneous"
        )
    )

    ## Cold mill process state switches

    deactivate_casting = process_step.process_state_handler.process_state_switch_selector_handler.process_state_switch_handler.create_process_state_switch_at_next_discrete_event(
        start_process_state=input_and_output_stream_providing_state,
    ...
    process_step.process_state_handler.process_state_switch_selector_handler.create_single_choice_selector(
        process_state_switch=deactivate_casting
    )
    activate_casting = process_step.process_state_handler.process_state_switch_selector_handler.process_state_switch_handler.create_process_state_switch_at_input_stream(
        start_process_state=batch_idle_state,
        end_process_state=input_and_output_stream_providing_state,
    )
    process_step.process_state_handler.process_state_switch_selector_handler.create_single_choice_selector(
        process_state_switch=activate_casting
    )

    # batch_to_conti_step.process_state_handler.process_state_switch_selector_handler.create_single_choice_selector(
    #     process_state_switch=batch_input_state_activation
    # )
